...
def get_comments(av, index=None):
    # f = open(av + '.txt', 'w')
    if index:
        url = 'http://bilibili.com/video/av{}/index_{}.html'.format(str(av), index)
    else:
    ...
    for each in content:
        ...
        if title:
            ...
            if cid_html_1 or cid_html_2:
                ...
                return comments_time_list, int(max_second), title
            else:
                print('error')
        else:
...
def process_time(comments_time_list, max_second, title):
...
if __name__ == '__main__':
    ...
    av = '3133255'
    comments_time_list, max_second, title = get_comments(av, 10)
    # comments_time_list = list(range(2800))
    # max_second = len(comments_time_list)
    process_time(comments_time_list, max_second, title)
